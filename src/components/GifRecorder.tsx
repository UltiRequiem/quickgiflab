...
export default function WebcamGifRecorder() {
  ...
  const handleRequestCamera = async () => {
    try {
      ...
      toast({ title: "Camera access granted!", description: "You can now start recording." });
    } catch (error) {
      toast({
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Record high-quality GIFs using your webcam. Perfect for reactions, tutorials, and sharing moments!
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-gray-50 rounded-lg">
          <div className="text-center">
            ...
            <p className="font-semibold">{recordingOptions.width}x{recordingOptions.height}</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            ...
            <p className="font-semibold">
              {recordingOptions.gifQuality <= 3 ? 'High' : recordingOptions.gifQuality <= 6 ? 'Medium' : 'Low'}
            </p>
          </div>
          <div className="text-center">
          ...
          {!isPermissionGranted ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
              ...
              <p className="text-sm opacity-75 text-center max-w-md">
                Click "Enable Camera" to start recording high-quality webcam GIFs
              </p>
            </div>
          ) : previewUrl ? (
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          {!isPermissionGranted ? (
            <Button onClick={handleRequestCamera} size="lg" className="flex items-center gap-2">
              <Camera size={20} />
              Enable Camera
            </Button>
          ) : !isRecording && !recordedBlob ? (
            <Button onClick={startRecording} size="lg" className="flex items-center gap-2 bg-red-500 hover:bg-red-600">
              <Video size={20} />
              Start Recording
            </Button>
          ) : isRecording ? (
        ...
        <div className="text-sm text-gray-600 bg-gray-50 p-4 rounded-lg">
          ...
          <ul className="space-y-1 list-disc list-inside">
            ...
            <li>Recording at {recordingOptions.width}x{recordingOptions.height} @ {recordingOptions.frameRate}fps for perfect timing</li>
            <li>GIF duration matches recording time exactly (3 seconds = 3 second GIF)</li>
            <li>Keep recordings under 10 seconds for best file size and quality</li>
            <li>Frame rate optimized for smooth playback without speed issues</li>
          </ul>
        </div>
      </CardContent>
    </Card>
